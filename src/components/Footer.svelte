<script>

</script>

<!-- Footer -->
<footer class="page-footer font-small container-fluid teal pt-4 bg-light">

    <!-- Footer Text -->
    <div class="container text-center text-md-left">

        <!-- Grid row -->
        <div class="row">

            <!-- Grid column -->
            <div class="col-12 col-md-6 mt-md-0 mt-3 der">

                <!-- Content -->
                <h5 class="text-uppercase contacto font-weight-bold">Contacto</h5>
                <a target="_blank" href="https://api.whatsapp.com/send?phone=543624260839">
                    <h6>
                        <i class="fab fa-whatsapp"></i>
                        3624260839 (ESTEFI)
                    </h6>
                </a>
                <a target="_blank" href="https://api.whatsapp.com/send?phone=543624260839">
                    <h6>
                        <i class="fab fa-whatsapp"></i>
                        3625241786 (ALE)
                    </h6>
                </a>
            </div>
            <!-- Grid column -->

            <hr class="clearfix w-100 d-md-none pb-3" />

            <!-- Grid column -->
            <div class="col-12 col-md-6 mb-md-0 mb-3 izq d-md-block d-none">

                <!-- Content -->
                <a target="_blank" href="mailto:ZARIGUEYACHACO@GMAIL.COM">
                    <h6>
                        ZARIGUEYACHACO@GMAIL.COM
                        <i class="far fa-envelope"></i>
                    </h6>
                </a>
                <a target="_blank" href="https://www.instagram.com/zarigueya_gestandoconciencia/">
                    <h6>
                        ZARIGUEYA_GESTANDOCONCIENCIA
                        <i class="fab fa-instagram"></i>
                    </h6>
                </a>
                <a target="_blank" href="https://www.facebook.com/zarigueyagestandoconciencia/">
                    <h6>
                        ZARIGUEYA: GESTANDO CONCIENCIA
                        <i class="fab fa-facebook-square"></i>
                    </h6>
                </a>

            </div>
            <div class="col-12 col-md-6 mb-md-0 mb-3 d-md-none d-block">

                <!-- Content -->
                <a target="_blank" href="mailto:ZARIGUEYACHACO@GMAIL.COM">
                    <h6>
                        ZARIGUEYACHACO@GMAIL.COM
                        <i class="far fa-envelope"></i>
                    </h6>
                </a>
                <a target="_blank" href="https://www.instagram.com/zarigueya_gestandoconciencia/">
                    <h6>
                        ZARIGUEYA_GESTANDOCONCIENCIA
                        <i class="fab fa-instagram"></i>
                    </h6>
                </a>
                <a target="_blank" href="https://www.facebook.com/zarigueyagestandoconciencia/">
                    <h6>
                        ZARIGUEYA: GESTANDO CONCIENCIA
                        <i class="fab fa-facebook-square"></i>
                    </h6>
                </a>

            </div>
            <!-- Grid column -->

        </div>
        <!-- Grid row -->

    </div>
    <!-- Footer Text -->

    <!-- Copyright -->

</footer>
<!-- Footer -->

<style>
    /* @import url("https://fonts.googleapis.com/css2?family=Comic+Neue:ital,wght@0,300;0,400;0,700;1,300;1,400;1,700&display=swap"); */

    footer {
        background: linear-gradient(
            90deg,
            rgb(252, 234, 188) 0,
            rgb(255, 247, 227) 70%,
            rgb(252, 234, 188)
        ) !important;
        font-family: 'Comic Neue';
    }
    h6 {
        padding: 10px 0;
        font-weight: 800;
        font-size: 16px;
    }
    @media (max-width: 600px) {
        .izq {
            display: none;
        }
    }
    .izq {
        text-align: right;
    }
    a {
        text-decoration: none;
        color: rgb(190, 86, 89) !important;
    }
    a:hover {
        text-decoration: none;
        color: #ff7a6d !important;
    }
    .contacto {
        color: rgb(190, 86, 89) !important;
    }
</style>
